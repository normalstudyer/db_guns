 ...
 *
 作者：哎_小羊_168
 来源：CSDN
 原文：https://blog.csdn.net/aixiaoyang168/article/details/49468337
...
public class ImageUtil {
     ...
     */
    public static void reduceImageRatio(String srcImagePath, String toImagePath, float ratio) throws IOException {
        ...
        try {
            ...
            int height = src.getHeight(); // 缩小边长
            BufferedImage tag = new BufferedImage((int) (width / ratio), (int) (height / ratio), BufferedImage.TYPE_INT_RGB); // 绘制 缩小后的图片
    ...
    public static void reduceImageByWidthHeight(String srcImagePath, String toImagePath, int width, int height) throws IOException {
        FileOutputStream out = null;
        try { //读入文件
            File file = new File(srcImagePath); // 构造Image对象
            BufferedImage src = javax.imageio.ImageIO.read(file); // 缩小边长
            BufferedImage tag = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB); // 绘制缩小后的图片
     ...
     */ public static void cutImage(String srcpath,String subpath,int x,int y,int width,int height) throws IOException {
         ...
         try { // 读取图片文件
             ...
             ImageReader reader = it.next(); // 获取图片流
             iis = ImageIO.createImageInputStream(is);
             reader.setInput(iis, true);
             ImageReadParam param = reader.getDefaultReadParam(); // 图片裁剪区域。Rectangle 指定了坐标空间中的一个区域，通过 Rectangle 对象的左上顶点的坐标(x，y)、宽度和高度可以定义这个区域。
     ...
     * @return
     * @throws IOException
     */ public static float getScaleCutImage(String srcImagePath) throws IOException{ //读入文件
        File file = new File(srcImagePath); // 构造Image对象
     ...
     * @param path 图片路径
     * @return
     */ public static boolean base64ToImage(String base64, String path) { // 图像数据为空
        if(base64 == null){
            return false;
        ...
        try{
            ...
            for(int i = 0; i<bytes.length; i++){ // 调整异常数据
